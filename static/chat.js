...
function sendReplica() {
    ...
    // Get only the last message, in its raw format
    const lastMessage = $('.human-replica:last').text();
    var phrase = lastMessage
    phrase = phrase.replace(/^Human:/, '');
    phrase = phrase.replace(/^Assistant:/, '');
    // Set up the message to be sent
    // const inputs = lastMessage; // No modifications, tags, or additional formatting
    ...
    // Set up the message to be sent
    const inputs = phrase
        // Update position (although not strictly necessary now)
    position = $('.human-replica').length + $('.ai-replica').length;
